...
export default async function Logo() {
  ...
  return (
    <LocalizedClientLink
      ...
      className={cn(
        tektur.className,
        'flex items-center gap-2 text-2xl text-foreground hover:text-violet-700 cursor-pointer'
      )}
      data-testid="nav-store-link"
    >
      <div
        dangerouslySetInnerHTML={{ __html: logoSvg }}
        className="size-5 sm:size-6 [&_path]:fill-violet-600"
      />
      <h1 className="flex items-center tracking-wide text-base sm:text-lg">
        <span className="font-semibold">fit</span>
        <span className="font-light text-violet-600">front</span>
      </h1>
    </LocalizedClientLink>
  );
}
